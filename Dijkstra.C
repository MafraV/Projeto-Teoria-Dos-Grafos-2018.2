...
void Dijk(int grafo[tam][tam], int origem, int destino)
{
	...
	while(origem != destino)
	{
		...
		{
			val1 = grafo[origem][i];
			val2 = grafo[cabeca][origem] + grafo[origem][i];
			/*printf("grafo[%d][%d] = %d e grafo[origem][i] = %d, na interação %d\n", cabeca, origem, grafo[cabeca][origem], grafo[origem][i], i);
			printf("val1 = %d e val2 = %d\n", val1, val2);*/
			if(val1 < dist[i] || val2 < dist[i])
			{
				if(aux[i] != 1)
				{
					dist[i] = val1 <= val2 ? val1 : val2;
					//printf("o valor de dist[%d] é %d\n", i, dist[i]);
...
int main()
{
	...
	scanf("%d%d%d", &v1, &v2, &v);
	if(v<0)
		printf("\nO algoritmo de Dijkstra não funciona para arestas com valores negativos");
	while(v>0)
	{
		grafo[v1][v2] = v;
		scanf("%d%d%d", &v1, &v2, &v);
